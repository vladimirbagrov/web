...
class SimpleCommand implements Command
{
    ...
    public function execute(): void
    {
        echo "SimpleCommand: Видите, я могу делать простые вещи, такие как печать (" . $this->payload . ")\n";
    }
}
...
class Receiver
{
    ...
    {
        echo "Приемник: Также работаю над (" . $b . ".)\n";
    }
}
...
class Invoker
{
    ...
    {
        echo "Приглашающий: Кто-нибудь хочет, чтобы что-то было сделано до того, как я начну?\n";
        if ($this->onStart instanceof Command) {
            $this->onStart->execute();
        }

        echo "Призывающий: ... делаю что-то действительно важное...\n";
...
echo "<pre>";
$invoker = new Invoker();
$invoker->setOnStart(new SimpleCommand("Поздоровайся!"));
$receiver = new Receiver();
$invoker->setOnFinish(new ComplexCommand($receiver, "Отправить электронное письмо", "Сохранить отчет"));
