...
function clientCode()
{
    ...
    if ($p1->primitive === $p2->primitive) {
        ...
        echo "Значения примитивных полей не были скопированы. Ого!\n";
    }
    if ($p1->component === $p2->component) {
        echo "Простой компонент не был клонирован. Ого!\n";
    } else {
        echo "Простой компонент был клонирован. Ура!\n";
    }

    if ($p1->circularReference === $p2->circularReference) {
        echo "Компонент с обратной ссылкой не был клонирован. Ого!\n";
    } else {
        echo "Компонент с обратной ссылкой был клонирован. Ура!\n";
    }

    if ($p1->circularReference->prototype === $p2->circularReference->prototype) {
        echo "Компонент с обратной ссылкой связан с исходным объектом. Ого!\n";
    } else {
