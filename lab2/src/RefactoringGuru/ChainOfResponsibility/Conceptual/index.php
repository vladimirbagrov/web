...
class MonkeyHandler extends AbstractHandler
{
    ...
    {
        if ($request === "банан") {
            return "Обезьяна: Я съем этот " . $request . ".\n";
        } else {
...
class SquirrelHandler extends AbstractHandler
{
    ...
    {
        if ($request === "орех") {
            return "Белка: Я съем этот " . $request . ".\n";
        } else {
            return parent::handle($request);
        }
    }
...
class DogHandler extends AbstractHandler
{
    public function handle(string $request): ?string
    {
        if ($request === "Фрикадельки") {
            return "Собака: Я съем эти " . $request . ".\n";
        } else {
            return parent::handle($request);
        }
    }
...
{
    foreach (["орех", "банан", "чашку кофе"] as $food) {
        echo "Клиент: Кто хочет " . $food . "?\n";
        $result = $handler->handle($food);
        if ($result) {
            echo "  " . $result;
        } else {
            echo "  " . $food . " остался нетронутым.\n";
        }
    }
}
